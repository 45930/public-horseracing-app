...
from app.service_objects.ml import RaceClassificationPredictor
...
# statement = open(
#     './scripts/ml/race_classification.sql').read().replace('{{date_input}}', f"'{str(predict_date)}'")
...
# predictions = RaceClassificationPredictor(data=df).call()
