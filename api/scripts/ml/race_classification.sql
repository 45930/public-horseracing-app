...
prediction_data AS (
  SELECT
    ...
    s.win_rate,
    r_avg.field_size,
    ROUND(s.best_speed_lifetime, 4) AS best_speed_lifetime,
    ROUND(s.avg_speed_lifetime, 4) AS avg_speed_lifetime,
    ROUND(s.comprehensive_perf, 4) AS comprehensive_perf,
    ROUND(s.best_speed_365, 4) AS best_speed_365,
    ROUND(s.avg_speed_365, 4) AS avg_speed_365,
    ROUND(s.purse_3, 4) AS purse_3,
    ROUND(s.best_speed_3, 4) AS best_speed_3,
    ROUND(s.eps, 4) AS eps,
...
SELECT
  ...
  td.horse_id,
  td.horse_name,
  td.best_speed_lifetime,
  td.avg_speed_lifetime,
  td.comprehensive_perf,
  td.best_speed_365,
  td.avg_speed_365,
  td.purse_3,
  td.best_speed_3,
  ...
  td.run_this_quarter,
  td.run_this_year,
  rank() OVER (PARTITION BY td.race_id ORDER BY td.post_position ASC) AS adj_post_position,
    layoff_gt_year,
