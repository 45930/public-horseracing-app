...
from app.models import Result, Trainer

time_range = int(os.environ.get('DFAULT_TIME_RANGE'))


class TrainerAPI(BaseModelApi):
    def __init__(self):
        pass

    def show(self, id):
        return json.dumps(db.session.query(Trainer).filter(Trainer.id == id).first().as_dict())

    def index(self):
        return json.dumps([row.as_dict() for row in db.session.query(Trainer).order_by('full_name').all()])
    ...
    def top(self, after=None):
        ...
        statement = f'''
            ...
            SELECT ar.trainer_id AS id, ar.full_name, count(ar.trainer_id) AS starts, COALESCE(sq.wins, 0) AS wins
            FROM applicable_results ar
                INNER JOIN races r ON ar.race_id = r.id
    ...
    def track_stats(self, id, date_input):
        statement = f'''
            WITH applicable_results AS (
                SELECT trainer_id, r.id as result_id, race_id, r.final_position
                FROM results r INNER JOIN races ra on r.race_id = ra.id
                WHERE trainer_id = {id} AND r.final_position > 0 AND ra.date > '{date_input}'
            ), track_wins AS (
                SELECT tracks.code, count(ar.trainer_id) AS wins
                ...
                WHERE ar.trainer_id = {id} AND res.final_position = 1 AND ra.date > '{date_input}'
                GROUP BY tracks.code
            )
            
            SELECT tracks.code AS track_code, count(ar.trainer_id) AS starts, COALESCE(sq.wins, 0) AS wins
            FROM applicable_results ar
    ...
    def jockey_stats(self, id, date_input):
        statement = f'''
            WITH applicable_results AS (
                SELECT trainer_id, r.id as result_id, race_id, final_position, jockey_id, j.full_name AS jockey_name
                FROM results r 
                    INNER JOIN races ra on r.race_id = ra.id
                    INNER JOIN jockeys j on r.jockey_id = j.id
                WHERE trainer_id = {id} AND final_position > 0 AND ra.date > '{date_input}'
            ), jockey_wins AS (
                SELECT ar.jockey_id, count(ar.trainer_id) AS wins
                FROM applicable_results ar 
            ...
            SELECT ar.jockey_name AS full_name, ar.jockey_id AS id, count(trainer_id) AS starts, COALESCE(sq.wins, 0) AS wins
            FROM applicable_results ar
    ...
    def class_stats(self, id, date_input):
        statement = f'''
            WITH applicable_results AS (
                SELECT trainer_id, r.id as result_id, race_id, final_position, ra.classification
                FROM results r INNER JOIN races ra on r.race_id = ra.id
                WHERE trainer_id = {id} AND final_position > 0 AND ra.date > '{date_input}'
            ), class_wins AS (
                SELECT ar.classification, count(ar.trainer_id) AS wins
                FROM applicable_results ar 
                    INNER JOIN results res ON res.id = ar.result_id
                WHERE res.final_position = 1
            ...
            SELECT ar.classification, count(trainer_id) AS starts, COALESCE(sq.wins, 0) AS wins
            FROM applicable_results ar
    ...
    def surface_stats(self, id, date_input):
        statement = f'''
            WITH applicable_results AS (
                SELECT c.id, c.surface, CASE WHEN c.distance >= 8 THEN 1 ELSE 0 END AS route, trainer_id, r.id as result_id, race_id, final_position
                FROM results r 
                    INNER JOIN races ra on r.race_id = ra.id
                    INNER JOIN courses c on ra.course_id = c.id
                WHERE trainer_id = {id} AND final_position > 0 AND ra.date > '{date_input}'
            ), surface_wins AS (
                SELECT ar.route, ar.surface, count(ar.trainer_id) AS wins
                FROM applicable_results ar 
                    INNER JOIN results res ON res.id = ar.result_id
                WHERE res.final_position = 1
            ...
            SELECT ar.route, ar.surface, count(trainer_id) AS starts, COALESCE(sq.wins, 0) AS wins
            FROM applicable_results ar
